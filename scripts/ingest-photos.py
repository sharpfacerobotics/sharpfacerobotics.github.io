...
# Interleave rather than list in file order. Ingest order clusters each batch
# together, so a new drop would all appear at one end of the carousel. A FIXED
# seed keeps the order stable between builds — a fresh shuffle on every render
# would make the carousel and the Contact wall reorder under the reader.
import random
random.Random(30450).shuffle(rel)
